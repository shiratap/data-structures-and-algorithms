...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(item => houses.push((Object.values(item))[3]));
  return houses;
}
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(item => {
    if(Object.values(item)[0] === character){
      if(Object.values(item)[2].length === 0){
        value = false;
      }
      else{value = true}
    }
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(item => {
    if(Object.entries(item)[0][1] === character){
      if(Object.entries(item)[2][1].length === 0){
        value = false;
      }
      else{value = true}
    }
